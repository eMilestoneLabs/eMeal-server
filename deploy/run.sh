...
reg firstpublish RO   "Live-Test-16 first-publish & attendance-window contracts — mealPricingLocked exposed on both group payloads (lock-step), a LOCKED group rejects a pricing flip with 400 MEAL_PRICING_LOCKED while a same-value echo still succeeds (WRITE-gated, rejection-only so nothing needs cleanup), and every active meal window is present and same-day — overlap/minimum-gap were WITHDRAWN 2026-08-03, so any number of concurrent windows is allowed (srs/firstpublish-windows.sh; read-only, never publishes and never consumes a lock)"
reg plannermodes RO   "Planner-mode contracts (Weekly / Day-Wise) — both mode flags exposed and MUTUALLY EXCLUSIVE while meals are ON, GET /groups/:id/meal-config agrees lock-step (a disagreement sends Schedule to the wrong planner), planner read reachable for the active mode (srs/planner-modes.sh; 100% read-only, creates nothing, publishes nothing, never consumes the first-publish lock)"
reg billinggate  RO   "Meal-Pricing billing gate — Meal Pricing is the master gate for Meal Billing, so a pricing-OFF group rejects billing-summary, my-billing, billing-series and the billing export with 400 BILLING_NOT_APPLICABLE while a priced group is unchanged and the ATTENDANCE export stays available (feature-preservation guard); a rejection-only WRITE probe proves a pricing-OFF group refuses a billing-cycle change (srs/billing-gate.sh)"
reg pubisolation RO   "P-01 published-schedule isolation — MASTER is configuration, DRAFT is configuration-in-progress, PUBLISHED is the permanent operational baseline: every published schedule exposes requiresRepublish (legacy configs are LABELLED, never rebuilt from Master), the locked 7-day contract shape survives, a published meal carries its resolved identity + window, /meals/today agrees with the planner on meal identity (divergence = the P-01 leak), and a foreign group is rejected rather than 200-empty (srs/published-isolation.sh; 100% read-only, never publishes, never consumes the first-publish lock)"
reg memberscope RO   "Group-scoped member settings — vacation and Personal Auto-Attendance are PER GROUP (they used to live only on users, so one toggle governed every group a member belonged to and an approved group-A vacation suppressed billing in group B): asserts the migration is applied, that NULL still means inherit so legacy rows are untouched, that GET /groups/:id exposes myMemberSettings as a per-group read source, that it carries the REQUESTER own overrides only (never every member state), that the list path stays detail-only, and that a foreign group id is rejected (srs/member-settings-scope.sh; 100% read-only, GETs + one psql SELECT, toggles nothing)"
...
run_module() { # $1 = name
  ...
  case "$m" in
    ...
    plannermodes) ( WRITE_TESTS=$WRITES bash deploy/srs/planner-modes.sh ) 2>&1 | tee "$log"; rc=${PIPESTATUS[0]};;
    billinggate) ( WRITE_TESTS=$WRITES bash deploy/srs/billing-gate.sh ) 2>&1 | tee "$log"; rc=${PIPESTATUS[0]};;
    pubisolation) ( bash deploy/srs/published-isolation.sh ) 2>&1 | tee "$log"; rc=${PIPESTATUS[0]};;
    memberscope) ( bash deploy/srs/member-settings-scope.sh ) 2>&1 | tee "$log"; rc=${PIPESTATUS[0]};;
...
# srs gated too (2026-07-19): its PERF-A section (health / attendance-today
